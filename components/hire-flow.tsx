...
const employeeTypes: Record<EmployeeRole, any> = {
  'HR Manager': {
...
const ProgressIndicator = ({ currentStep, isExistingWorkspace }: { currentStep: number, isExistingWorkspace: boolean }) => {
    // MODIFIED: ProgressIndicator now adapts based on the context
    const steps = isExistingWorkspace ? ['Select Role', 'Define Tasks'] : ['Setup Workspace', 'Select Role', 'Define Tasks'];
    const progressPercentage = isExistingWorkspace
      ? (currentStep > 1 ? ((currentStep - 1) / (steps.length)) * 100 : 0)
    ...
    <div id="progress-indicator-container" className="w-full px-2 sm:px-8 mb-16">
      <div className="relative flex items-center justify-between">
        ...
        {steps.map((step, index) => {
          ...
          const isActive = isExistingWorkspace ? currentStep >= stepNumber : currentStep >= stepNumber;
...
// We wrap the component to use useSearchParams
const HireFlowWrapper = () => (
...
function HireFlow() {
  ...
  // MODIFIED: State to handle existing workspace context
  ...
  // MODIFIED: Start at step 1 by default, but check URL params
  ...
  useEffect(() => {
    ...
    // MODIFIED: Logic to skip to step 2 if workspaceId exists
    if (companyId && workspaceId) {
      setExistingCompanyId(companyId);
      setExistingWorkspaceId(workspaceId);
      setStep(2); // Start at role selection
    }
  ...
  const handleEmployeeSelect = (type: EmployeeRole) => {
    ...
    setStep(3); // Go to task definition
  };

  // handleTaskChange, addCustomTask, handleWorkspaceChange remain the same
  ...
  // MODIFIED: Navigation logic needs to account for the starting step
  const nextStep = () => setStep(prev => prev + 1);
  const prevStep = () => {
    // Can't go back from role selection if we started there
    if (existingWorkspaceId && step === 2) {
  ...
  const handleHire = async () => {
    ...
    try {
      let companyIdToUse = existingCompanyId;
      let workspaceIdToUse = existingWorkspaceId;

      // MODIFIED: This block only runs for NEW workspaces
      if (!companyIdToUse || !workspaceIdToUse) {
        console.log("Creating new company and workspace...");
        ...
        workspaceIdToUse = newWorkspace.id;
      }

      // This part runs for both new and existing workspaces
      console.log(`Adding employee to workspace: ${workspaceIdToUse}`);
      ...
      console.log(`Redirecting to: /dashboard/${companyIdToUse}/${workspaceIdToUse}`);
      router.refresh();
      router.push(`/dashboard/${companyIdToUse}/${workspaceIdToUse}`);
  ...
  // The rest of the JSX remains largely the same, but we conditionally render the steps
  const isExistingFlow = !!existingWorkspaceId;
    ...
    <div id="hire-flow-page-container" className="min-h-screen bg-gray-50 text-foreground flex items-center justify-center p-4">
      <div id="hire-flow-main-card" className="w-full max-w-7xl mx-auto p-8 sm:p-12">
        <AnimatePresence mode="wait">
          ...
          >
            {/* MODIFIED: Pass prop to ProgressIndicator */}
            <ProgressIndicator currentStep={step} isExistingWorkspace={isExistingFlow} />

            {/* Step 1: Workspace Setup (Only for new users) */}
            {step === 1 && !isExistingFlow && (
        ...
        <div id="navigation-controls" className="flex justify-between items-center mt-10 pt-6 border-t border-border">
          {/* MODIFIED: Don't show back button on step 1 or on step 2 if existing */}
          {(step > 1 && !isExistingFlow) || (step > 2 && isExistingFlow) ? (
            <Button
              id="back-button"
              onClick={prevStep}
              variant="outline"
          ...
          {step < 3 ? (
            ...
            <Button
              ...
              disabled={isLoading}
            >
              {isLoading ? 'Hiring...' : 'Complete Setup & Hire'}
            </Button>
          )}
        </div>
      </div>
...
export default HireFlowWrapper;